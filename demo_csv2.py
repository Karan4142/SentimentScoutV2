...
def login():
  if request.method == 'POST':
    ...
    if request.form['username'] == 'Karan' and request.form[
        'password'] == '12345':
...
def home_with_sentiment():
  ...
  if request.method == 'POST':
    ...
    sentiment_counts, positive_keywords, negative_keywords = get_sentiment_distribution(
        comments)
...
def extract_keywords(text, num_keywords=1):
  ...
  words = [word for word in words if word.isalnum() and len(word) > 1
           ]  # Only alphanumeric characters with length > 1
...
def preprocess_comment(comment):
  ...
  comment['text'] = re.sub(r'br', '', comment['text'])  # Remove br
  comment['text'] = re.sub(r'\b\d+\b', '',
                           comment['text'])  # Remove numeric values
  ...
  while len(comments) < max_comments:
    ...
    for item in data['items']:
      comment_text = item['snippet']['topLevelComment']['snippet'][
          'textDisplay']
      comment_time_str = item['snippet']['topLevelComment']['snippet'][
          'publishedAt']
...
def save_comments_to_csv(comments, csv_filename='comments.csv'):
  with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['time', 'text', 'sentiment',
                  'keywords']  # Include 'keywords' in fieldnames
...
def create_pie_chart(sentiment_data):
  ...
  plt.savefig(
      'static/images/sentiment_pie_chart.png')  # Save the chart as an image
  plt.close()
...
def create_time_series_plot(comments):
  ...
  for comment in comments:
    sentiment = comment.get('sentiment', 'Neutral')
    sentiment_counts[sentiment] += 1
    time_series_data[sentiment].append(
        (comment['time'], sentiment_counts[sentiment]))
  ...
  min_time = min(
      min(data, key=lambda x: x[0])[0] for data in time_series_data.values())
  max_time = max(
      max(data, key=lambda x: x[0])[0] for data in time_series_data.values())
  ...
  plt.gca().xaxis.set_major_locator(
      mdates.DayLocator(interval=days_interval // 10))
